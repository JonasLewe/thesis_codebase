...
#parent_dir_name = "thin_section_dataset_ppl_0"
#parent_dir_name = "thin_section_dataset_ppl_30"
#parent_dir_name = "thin_section_dataset_ppl_60"

#parent_dir_name = "thin_section_dataset_xpl_0"
#parent_dir_name = "thin_section_dataset_xpl_30"
parent_dir_name = "thin_section_dataset_xpl_60"

img_type = parent_dir_name.split("_")[-2]

rotation_ids = ["001", "007", "013"] # representing 0°, 30°, 60° rotation angle

rotation_id = rotation_ids[2]


# Sample file path: "../../../Data/Input_Data/raw_img_data/15666\r2"
def create_single_view_dataset(raw_img_src,
                                    parent_dir_name,
                                    class0="non-olivine",
                                    class1="olivine",
                                    save_path=save_path,
                                    split_data_xlsx=split_data_xlsx,
                                    img_type=img_type,
    ...
    for subdir, dirs, files in os.walk(raw_img_src):
        if os.path.normpath(subdir).split(os.sep)[-1] in section_ids: # ignore folders that are not on the deepest level
            ...
            raw_files_list = [img_id for img_id in os.listdir(subdir) if f"_{img_type}{rotation_id}" in img_id]
            img_stack = get_img_stack(subdir, raw_files_list, img_type=img_type) # should return list with one element
            # max_img = merge_img_stack_to_max(img_stack)
            rotated_img = Image.fromarray(img_stack[0])

            # 4. Step: Save img to appropriate folder
            rotated_img.save(os.path.join(save_path, parent_dir_name, split_folder, img_class, img_name), "JPEG", quality=100)
            print(f"Saved image: {img_name} to {split_folder}/{img_class}.")
    print(f"No. of files: {counter}")

if __name__ == "__main__":
    create_single_view_dataset(raw_img_src, parent_dir_name, img_type=img_type, rotation_id=rotation_id)
